...
As $g_{00} \to 0$ (near the event horizon of a financial black hole), $V_{obs} \to 0$.
**Conclusion:** The money is moving fast *inside* the financial sector ($V_{loc}$ is high), but it is frozen relative to the real economy ($V_{obs} \approx 0$).
...
In the holographic dual, Money Velocity $V$ corresponds to the **Information Scrambling Rate** (Lyapunov Exponent $\lambda_L$) on the boundary.
$$ V \sim \lambda_L $$
...
*   **Result:** The effective velocity drops exponentially with Complexity ($C$).
$$ V_{obs} \propto e^{-C} $$
