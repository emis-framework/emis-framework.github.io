...
- Extended country list (60 countries)
...
COUNTRY_CODE_MAP = {
    ...
    # 新增 30 个
    'IRN': '364', 'EGY': '818', 'PAK': '586', 'COL': '170', 'PHL': '608',
    'VNM': '704', 'NGA': '566', 'BGD': '050', 'UKR': '804', 'ROU': '642',
    'CZE': '203', 'HUN': '348', 'GRC': '300', 'PRT': '620', 'ISR': '376',
    'KAZ': '398', 'CHL': '152', 'PER': '604', 'FIN': '246', 'DNK': '208',
    'NZL': '554', 'QAT': '634', 'ARE': '784', 'OMN': '512', 'KWT': '414',
    'LKA': '144', 'MAR': '504', 'TWN': '158', 'HKG': '344', 'IRQ': '368'
...
def download_worldbank_gdp(year):
    cached = load_cache('gdp', params={'year': year,'num_countries': num_countries})
    if cached is not None:
        return cached
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if len(data) > 1 and isinstance(data[1], list):
                ...
                save_cache(df, 'gdp', params={'year': year,'num_countries': num_countries})
                return df
...
full_df['log_trade'] = np.log(full_df['trade']+1) # 对数变换，避免负值
...
def ppml_estimate(df):
    """
    PPML gravity estimation with exporter and importer fixed effects.
    Specification:
    ...
    print("\nRunning PPML estimation with Exporter & Importer FE...")
    ...
    # 不删除 0 贸易
    data['trade'] = data['trade'].clip(lower=0)

    # log(distance)
    data['ln_distance'] = np.log(data['dist'])

    try:
        model = smf.glm(
            formula='trade ~ ln_distance + C(iso_o) + C(iso_d)',
            data=data,
            ...
            method='newton',
            cov_type='cluster',
            cov_kwds={'groups': data['iso_o']}  # exporter cluster
        )

        # 计算 beta
        beta = model.params['ln_distance']
        
        # 计算 pseudo-R²
        pseudo_r2 = 1 - model.deviance / model.null_deviance  # 新增 pseudo-R²
        

        print("PPML Results (with FE):")
        print(f"  Distance exponent β = {abs(beta):.3f}")
        print(f"  Std Error = {model.bse['ln_distance']:.3f}")
        print(f"  z-stat = {model.tvalues['ln_distance']:.2f}")
        print(f"  Pseudo-R² = {pseudo_r2:.3f}")  # 新增 pseudo-R²
        print(f"  Observations: {int(model.nobs)}")

        # 返回结果，增加了 pseudo-R² 和模型系数
        return {
            'beta': abs(beta),
            'std_error': model.bse['ln_distance'],
            'r_squared': pseudo_r2,  # 返回 pseudo-R²
            'n': int(model.nobs),  # 返回样本量
            'df': data,            # 返回数据
            'coeffs': model.params  # 返回模型系数
        }

    except Exception as e:
        print("PPML estimation failed:")
        print(str(e))
        return None



print("\nRunning PPML estimation...")

ppml_result = ppml_estimate(full_df)
beta_dist_ppml = ppml_result['beta']  # 或者 -coeff 取绝对值

# 输出 PPML 结果
print(f"PPML Results:")
print(f"  Distance exponent β = {beta_dist_ppml:.3f}")
print(f"  Std Error = {ppml_result['std_error']:.3f}")
print(f"  Pseudo-R² = {ppml_result['r_squared']:.3f}")  # 输出 pseudo-R²
print(f"  Observations: {ppml_result['n']}")

results = {
    'beta': beta_dist_ppml,
    'se': ppml_result['std_error'],
    'r_squared': ppml_result['r_squared'],
    'n': ppml_result['n'],
    'df': full_df,
    'coeffs': ppml_result['coeffs']
}
...
# ==================== 修改开始：用真正 PPML 做 bootstrap ====================
print("Running bootstrap analysis (PPML)...")
beta_bootstrap = []
for _ in range(100):
    # 1️⃣ bootstrap 重采样
    ...
    # 2️⃣ 保留 distance > 0
    sample = sample[sample['dist'] > 0].copy()
    
    # 3️⃣ ln_distance
    sample['ln_distance'] = np.log(sample['dist'])
    ...
    try:
        model_boot = smf.glm(
            ...
            data=sample,
            family=sm.families.Poisson()
        ).fit(maxiter=500, method='newton')
        
        # 5️⃣ 取 ln_distance 系数作为 beta
        beta_bootstrap.append(abs(model_boot.params['ln_distance']))
...
output_pdf = os.path.join(OUTPUT_DIR, f'trade_gravity_fit_ppml_{YEAR}_{len(COUNTRIES)}.pdf')
output_png = os.path.join(OUTPUT_DIR, f'trade_gravity_fit_ppml_{YEAR}_{len(COUNTRIES)}.png')
