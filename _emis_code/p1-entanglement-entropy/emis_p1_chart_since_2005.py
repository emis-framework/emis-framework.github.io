...
# 缓存文件
US_INDEX_CACHE = os.path.join(CACHE_DIR,'sp500.csv')
US_ENTROPY_CACHE = os.path.join(CACHE_DIR,'entropy_US_since2005.csv')  
JP_INDEX_CACHE = os.path.join(CACHE_DIR,'index_Japan_since2005.csv')
JP_ENTROPY_CACHE = os.path.join(CACHE_DIR,'entropy_Japan_since2005_v2.csv')  
DE_INDEX_CACHE = os.path.join(CACHE_DIR,'index_DAX.csv')
DE_ENTROPY_CACHE = os.path.join(CACHE_DIR,'entropy_DAX_historical.csv')  

# 输出文件
FIG1 = os.path.join(DATA_FIGURE_DIR,'figure1_entropy_timeseries.pdf')
FIG2 = os.path.join(DATA_FIGURE_DIR,'figure2_return_distribution.pdf')
FIG3 = os.path.join(DATA_FIGURE_DIR,'figure3_cross_market.pdf')
FIG4 = os.path.join(DATA_FIGURE_DIR,'figure4_emis_vs_vix.pdf')
...
def load_data():
    ...
    # 美国
    data['US'] = {
        'entropy': pd.read_csv(US_ENTROPY_CACHE, index_col=0, parse_dates=True),
        'index': pd.read_csv(US_INDEX_CACHE, index_col=0, parse_dates=True)
    }
    
    # 日本
    data['Japan'] = {
        'entropy': pd.read_csv(JP_ENTROPY_CACHE, index_col=0, parse_dates=True),
        'index': pd.read_csv(JP_INDEX_CACHE, index_col=0, parse_dates=True)
    }
    
    # 德国
    data['Germany'] = {
        'entropy': pd.read_csv(DE_ENTROPY_CACHE, index_col=0, parse_dates=True),
        'index': pd.read_csv(DE_INDEX_CACHE, index_col=0, parse_dates=True)
    }
...
def create_figure1(data, save_path=FIG1):
    """
...
def create_figure2(data, save_path=FIG2):
    """
    Figure 2: 收益分布对比
    """
...
def create_figure3(data, save_path=FIG3):
    """
    Figure 3: 跨市场验证
    """
...
def create_figure4(save_path=FIG4):
    """
