...
# 输出文件
RESULT_3X3 = os.path.join(DATA_FIGURE_DIR,'paper_results_complete.csv')
...
def main():
    ...
    if os.path.exists(VIX_CACHE) and os.path.exists(US_INDEX_CACHE):
        vix = pd.read_csv(VIX_CACHE, index_col=0, parse_dates=True).iloc[:, 0]
        vix = fix_timezone(vix)
        
        sp500 = pd.read_csv(US_INDEX_CACHE, index_col=0, parse_dates=True).iloc[:, 0]
    ...
    # 保存
    df.to_csv(RESULT_3X3, index=False)
    print(f"\n结果已保存: {RESULT_3X3}")
